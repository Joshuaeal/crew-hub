...
export async function GET(_req: Request, ctx: Ctx) {
  const gate = await requireAnyPermission(["projects_manage", "billing"]);
  if (!gate.ok) return gate.response;

  const docs = await readBillingInvoicesForProject(ctx.params.slug);
...
export async function POST(req: Request, ctx: Ctx) {
  ...
  const doc = await createBillingInvoice({
    kind,
    customerName,
    customerEmail,
